...
function PlayingList({listTrack,onPlay}) {
  return (
    // move main page
          ...
          <div tabIndex={0} style={{
            ...
            >
                <div className={cx('scroll-child')}>
                    <div className={cx("list-item-container")} style={{marginTop:"0px"}}>
                        {/* map-list item */}
                        <div className={cx("list-item-parent")}>
                          {listTrack.map(track =>(
                            <div className={cx('item-full-left')} key={track.id}>
                            <Media title={track.title} img={track.img} audio={track.audioSrc}/>
                        </div>
                          ))}
